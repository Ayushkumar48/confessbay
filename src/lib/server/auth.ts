...
import { db, s3Client } from '$lib/server/db';
...
export async function validateSessionToken(token: string) {
	...
	const [result] = await db
		.select({
			...
			user: {
				...
				authProvider: table.user.authProvider,
				authProviderId: table.user.authProviderId,
				dateOfBirth: table.user.dateOfBirth,
				avatar: table.user.avatar,
				collegeId: table.user.collegeId,
				collegeEmail: table.user.collegeEmail,
				emailVerified: table.user.emailVerified,
				phoneVerified: table.user.phoneVerified,
				zodiacSign: table.user.zodiacSign,
	...
	const signedUrl = await getSvgUrl(user.avatar);
...
export async function getSvgUrl(key: string) {
	...
	return await getSignedUrl(s3Client, command, { expiresIn: 60 * 60 * 1 });
}
